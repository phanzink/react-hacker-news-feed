...
class News extends Component {
    ...
    loadMoreArticles ({ startIndex, stopIndex }) {
        if (startIndex >= 30) {
            ...
            articleIds.slice(startIndex, stopIndex).map(id => this.gatherArticles(id) );
        }
    }

    renderArticles() {
        const { articles } = this.state;
        return articles.map((article, key) => <div key={key}>testing</div>);
    }
